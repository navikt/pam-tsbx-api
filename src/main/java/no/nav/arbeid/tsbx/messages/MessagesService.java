...
public class MessagesService {
    ...
    private List<Message> mockMessages(UserId user) {
        ...
        for (int i=0; i<nMessages; i++) {
            final String title = titles[rng.nextInt(Integer.MAX_VALUE) % titles.length];
            final String employer = employers[rng.nextInt(Integer.MAX_VALUE) % employers.length];
            final String text = texts[rng.nextInt(Integer.MAX_VALUE) % texts.length];
