 ...
 * This server represents "Tokendings" used for token exchanges by consumer clients of this api.
 *
 * This is only for use in automated tests.
 */
public class MockOauthTokenExchangeServerInitializer implements ApplicationContextInitializer<GenericApplicationContext> {

    public static final String MOCK_OAUTH2_SERVER_PORT_PROPERTY_NAME = "mock-oauth2-server.port";

    private static final Logger LOG = LoggerFactory.getLogger(MockOauthTokenExchangeServerInitializer.class);
    ...
    public void initialize(GenericApplicationContext applicationContext) {
        try {
            ...
            applicationContext.registerBean(MockOAuth2Server.class, () -> server, def -> def.setDestroyMethodName("shutdown"));

            applicationContext.getEnvironment().getPropertySources().addLast(
                    new MapPropertySource(MockOauthTokenExchangeServerInitializer.class.getName(),
